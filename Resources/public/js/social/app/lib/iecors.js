define(['jquery', 'easyXDM'], function(jQuery){
    function isIE(){
    ...
    (function ($) {
        ...
        var doRequests = function (provider_base_url,callbacks) {
            if ( isIE() ) {
                var easyXDM_url = provider_base_url + "/easyXDM.min.js";
                if (easyXDM_debug) {
                    easyXDM_url = provider_base_url + "/easyXDM.debug.js"
                }
                function continue_after_easyXDM_load() {
                    ...
                    var remote_url = provider_base_url + "/cors/index.html";
                    ...
                    easyXDM_connection = new scoped_easyXDM.Rpc(
                        {
                            remote:remote_url,
                            swf:provider_base_url + "/easyxdm.swf",
                            container: container,
                ...
                $.getScript(easyXDM_url, function () {
                    ...
                    if (!(typeof(window["JSON"]) == 'object' && window["JSON"])) {
                        $.getScript(provider_base_url + "/json2.js", continue_after_easyXDM_load);
                    } else {
        ...
        $.ajaxTransport("+*",function (options, originalOptions, jqXHR) {
            if ( isIE() ) {
                ...
                var provider_base_url = window.api_server;
                        ...
                        jquery_easyXDM.getConnection(provider_base_url,{
                            success:function (easyXDM_connection) {
                                ...
                                for(header in headers){
                                    originalOptions.headers[header] = headers[header];
                                }

                                console.log('those were the headers of our lifes');
                                console.log(originalOptions.headers);
